...
def get_pie_chart(entered_site):
    ...
    if entered_site =='ALL':
        fig= px.pie(spacex_df, values='class', names='Launch Site', title='Successful Launch Count on Sites')
    else:
        filtered_df=spacex_df[spacex_df['Launch Site']==entered_site]
        filtered_df= filtered_df.groupby('class').count().reset_index()
        fig= px.pie(filtered_df, values='class', names='Site',title='Launch Count on Sites')
    return fig
# TASK 4:
...
def get_scatter_chart(entered_site1, payload_range):
    ...
    if entered_site1 =='ALL':
        filtered_df1 = spacex_df[(spacex_df['Payload Mass (kg)'] >= int(payload_range[0])) &
